...
export function TimelinePanelVariant1({
  ...
  return (
    <div className="rounded-[22px] bg-[linear-gradient(135deg,rgba(38,92,255,0.42),rgba(230,129,57,0.22)_48%,rgba(255,255,255,0.11))] p-px">
      <section className="rounded-[21px] border border-white/15 bg-white/[0.08] px-3.5 py-3 shadow-panel backdrop-blur-xl">
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <DateField selectedDate={selectedDate} onSelectedDateChange={onSelectedDateChange} />
            <ArrowPair onPreviousDay={onPreviousDay} onNextDay={onNextDay} />
          </div>
        </div>
      </section>
    </div>
...
function DateField({ selectedDate, onSelectedDateChange }: Pick<TimelinePanelVariantProps, "selectedDate" | "onSelectedDateChange">) {
  return (
...
function ArrowPair({ onPreviousDay, onNextDay }: Pick<TimelinePanelVariantProps, "onPreviousDay" | "onNextDay">) {
  return (
